...
function normalizeGeneratedManifest(input: unknown): GeneratedProjectFile[] | undefined {
  ...
  for (const file of files) {
    ...
    if (!isSafeRelativeProjectPath(pathValue)) return
    normalized.push({
      path: pathValue,
        ...
        try {
          ...
          try {
            ...
            for (let attempt = 1; attempt <= MAX_GENERATION_ATTEMPTS; attempt++) {
              ...
              const issues = validateProjectContract(files, template)
              if (issues.length === 0) {
                return { files }
              }
              ...
              await SessionPrompt.prompt({
                ...
                agent: "build",
                system: GENERATE_PROJECT_SYSTEM_PROMPT,
                parts: [{ type: "text", text: renderContractRepairPrompt(template, issues, attempt + 1, MAX_GENERATION_ATTEMPTS) }],
              })
            }
...
export const validateGeneratedProjectFiles = validateProjectContract
